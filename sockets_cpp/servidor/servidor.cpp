...
void * send_client(void * m) {
	...
	while(1) {
		if(!tcp.is_online() && tcp.get_last_closed_sockets() == desc->id) {
			cerr << "CONEXION FINALIZADA: DETENIENDO send_clients( id:" << desc->id << " ip:" << desc->ip << " )"<< endl;
			break;
		}
			...
			if( desc[i] )
			{
				if(!desc[i]->enable_message_runtime) 
				{
					desc[i]->enable_message_runtime = true;
						...
						cerr << "HILO NUEVO THREAD ENVIA MENSAJE" << endl;
					}
					num_message++;
				...
				cout << "id:      " << desc[i]->id      << endl
				     << "ip:      " << desc[i]->ip      << endl
				     << "mensaje: " << desc[i]->message << endl
				     << "socket:  " << desc[i]->socket  << endl
				     << "habilitado:  " << desc[i]->enable_message_runtime << endl;
				tcp.clean(i);
			}
		}
		usleep(1000);
...
{
	if(argc < 2) {
		cerr << "USO: ./servidor puerto segundosrespuesta" << endl;
		return 0;
	}
	if(argc == 3)
	...
	if( tcp.setup(atoi(argv[1]),opts) == 0) {
		...
		cerr << "ERROR AL ABRIR SOCKET" << endl;
	return 0;
}
